...
//--------
// TKGoRed
// Toggles between green/red based on a not false/false boolean
// (c) Keith Porcaro - 2016 - MIT License

 Tangle.classes.TKGoRed = {
            update: function(element, value){
                if (value == false){
...
Tangle.formats.household = function (value) {    
    if (value==1) {return "alone"}
    else if (value==2) {return "with "+ (value-1) + " other person"}
    else {return "with " + (value-1) + " other people"};
};
Tangle.formats.income = function (value) {    
    if (value==1) {return "my"}
    else {return "our"};
};

Tangle.formats.timeWindow = function (value) {     
    var times = ['week', 'month', 'year']
    return times[value]
};
